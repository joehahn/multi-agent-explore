...
def update_agents(state, agent, action, environment):
    ...
    state_next['bucket_productivity'] = calculate_bucket_productivity(environment, weather)
    #update weather
    weather, N_turns_weather = update_weather(state, environment)
    state_next['N_turns_weather'] = N_turns_weather
    state_next['weather'] = weather
    #update lagged quantities
    state_next['previous_weather'] = state['weather']
...
def update_weather(state, environment):
    ...
    if (N_turns_weather < 0):
        weather = 'nominal'
        N_turns_weather = 0
    return weather, N_turns_weather
...
def state2vector(state, environment):
    ...
    #items = [locations, agent_value, previous_bucket_productivity, weather_array]
    items = [locations, previous_bucket_productivity, weather_array]
    v = np.concatenate(items)
...
def experience_replay(memories, batch_size, environment, N_inputs, gamma, model):
    ...
    for idx in range(batch_size):
        ...
        action = actionz[idx]
        Qz[idx, action] = reward + gamma*max_Q_next
...
def train(environment, model, N_training_games, N_validation_games, gamma, good_memories, batch_size,
    ...
    for N_game in games:
        ...
        while (game_state == 'running'):
            ...
            game_memories += [(turn, state, agent, action, state_next, reward, game_state)]
