...
def initialize_environment(rn_seed, max_turns, N_buckets, N_agents):
    ...
    for idx in range(N_buckets):
        r0 = np.random.uniform(-1.0, 5.0)
        drdt = 0*np.random.uniform(-8.0, 12.0)/max_turns
        Pmin = max_turns/40.0
        Pmax = max_turns/4.0
...
def initialize_state(environment, turn):
    ...
    agent_locations = np.random.choice(all_buckets, size=N_agents, replace=False).tolist()
    ...
    state['previous_bucket_rewards'] =  get_bucket_rewards(environment, turn - 1)
    return state

#increment time and update bucket rewards
def get_bucket_rewards(environment, turn):
    bucket_rewards = []
    for b_params in environment['bucket_params']:
        reward = bucket_reward(b_params, turn)
        bucket_rewards += [reward]
    return bucket_rewards

#reward generated by a bucket
def bucket_reward(b_params, turn):
...
def state2vector(state, environment):
    ...
    l = [0.0]*N_buckets
    for agent_location in state['agent_locations']:
        l[agent_location] += 1.0
    l += state['previous_bucket_rewards']
    v = np.array(l).reshape(1, 2*N_buckets)
    return v
